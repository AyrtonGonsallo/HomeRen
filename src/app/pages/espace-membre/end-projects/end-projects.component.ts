...
@Component({
  selector: 'app-end-projects',
  templateUrl: './end-projects.component.html',
  styleUrl: './end-projects.component.css'
})
export class EndProjectsComponent {
 isconnected=false;
  user_id=0
  ...
  get_current_projects(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this.userService.getEndedProjetsByUserId(this.user_id).subscribe(
        (items) => {
          
          this.listOfCurrentProjects = items;
          console.log('Projets finis ', this.listOfCurrentProjects);
